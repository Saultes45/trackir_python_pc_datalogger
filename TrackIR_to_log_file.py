...
maxLogFileMegaBytesSize        = 100  # in [MB]
...
# data print format (not yet implemented)
rollLogFormat   = '.5f'
pitchLogFormat  = '.5f'
yawLogFormat    = '.5f'
xLogFormat      = '.5f'
...
def main():
    ...
    def signal_handler(sig, frame):
        trackrIr.stop()
        log.info("Number of frames logged (num_logged_frames): {}".format(num_logged_frames))
        log.info("Number of missed frames (num_missed_frames): {}".format(num_missed_frames))
        end_time = time.time()
        log.info("Total time (computer clock): {} s".format(round(end_time - start_time)))
        log.info(
    ...
    while (True):
        ...
        if trackIRData.frame != previous_frame:  # check the data are fresh using the frame number variable, reported by TrackIR
            ...
            if previous_frame != -1:
                num_missed_frames += trackIRData.frame - previous_frame - 1
            ...
            try:
                ...
                # pitchLogFormat = '.5f'
                # yawLogFormat = '.5f'
                log.info(',{},{},{:.5f},{:.5f},{:.5f},{:.5f},{:.5f},{:.5f}'.format(time_ms,
                                                                                         trackIRData.frame,
                                                                                         trackIRData.roll,
                                                                                         trackIRData.pitch,
                                                                                         trackIRData.yaw,
